...
export function ClassicTemplate({ data, showWatermark = false }: Props) {
          ...
          <View style={styles.section}>
            ...
            {data.projects.map((proj) => (
              <View key={proj.id} style={styles.entryBlock}>
                ...
                {proj.url && <Text style={styles.entrySubtitle}>{proj.url}</Text>}
                {proj.description && <Text style={styles.bulletText}>{proj.description}</Text>}
                {proj.technologies.length > 0 && (
                  <Text style={styles.entrySubtitle}>{proj.technologies.join(", ")}</Text>
                )}
              </View>
            ))}
          </View>
            ...
            <View key={section.id} style={styles.section}>
              ...
              {section.items.map((item) => (
                <View key={item.id} style={styles.entryBlock}>
                  ...
                  {item.subheading && <Text style={styles.entrySubtitle}>{item.subheading}</Text>}
                  {item.description && <Text style={styles.bulletText}>{item.description}</Text>}
                </View>
              ))}
            </View>
          )
